...
-- Hybrid Search Vector Setup
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'chat_memory_embeddings') THEN
        ALTER TABLE public.chat_memory_embeddings ADD COLUMN IF NOT EXISTS fts tsvector GENERATED ALWAYS AS (to_tsvector('english', content_summary)) STORED;
