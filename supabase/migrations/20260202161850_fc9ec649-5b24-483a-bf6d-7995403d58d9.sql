-- Create or replace the check_and_increment_usage function with credit fallback logic
...
BEGIN
  ...
  -- 8. PRIORITY 2: Try Pro bonus credits (if Pro user)
  IF v_is_pro AND v_pro_credits > 0 THEN
    UPDATE public.user_credits
    SET pro_bonus_credits = pro_bonus_credits - 1, 
        total_spent = total_spent + 1, 
        updated_at = NOW()
    WHERE user_id = p_user_id
    RETURNING balance + COALESCE(pro_bonus_credits, 0) INTO v_new_balance;
    
    -- Log the transaction
    INSERT INTO public.credit_transactions (user_id, credits, transaction_type, balance_after, description)
    VALUES (p_user_id, -1, 'usage', v_new_balance, p_feature_key || ' - Pro bonus credit used');
  ...
  IF v_credit_balance > 0 THEN
    ...
    -- Log the transaction
    INSERT INTO public.credit_transactions (user_id, credits, transaction_type, balance_after, description)
    VALUES (p_user_id, -1, 'usage', v_new_balance, p_feature_key || ' - Credit balance used');
  ...
  RETURN jsonb_build_object(
    ...
    'has_personal_key', v_has_personal_key,
    'credits_remaining', 0,
    'message', 'Both daily credits and credit balance exhausted.'
  );
END;
$$;