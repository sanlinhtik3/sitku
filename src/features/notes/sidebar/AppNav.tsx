...
const NAV_ITEM = "group flex w-full items-center gap-2.5 rounded-[10px] px-2.5 py-1.5 text-left text-[13px] transition-colors duration-[130ms] hover:bg-[#1a1a1c] hover:text-[#ededed]";
...
export function AppNav({ onOpenConsultant, onOpenCfo, activeRoom }: AppNavProps) {
  return (
    <nav className="shrink-0 border-b border-[rgba(255,255,255,0.05)] p-[2px_10px_8px] flex flex-col gap-[1px]">
      <button type="button" onClick={onOpenConsultant} className={`${NAV_ITEM} ${activeRoom === "consultant" ? "bg-[#1a1a1c] text-[#ededed] font-medium" : "text-[#c4c4c6]"}`}>
        <span className={NAV_TILE}><CaseMinimalistic className="h-[15px] w-[15px]" /></span>
        <span className="truncate">Agent Consultant</span>
      </button>
      <button type="button" onClick={onOpenCfo} className={`${NAV_ITEM} ${activeRoom === "cfo" ? "bg-[#1a1a1c] text-[#ededed] font-medium" : "text-[#c4c4c6]"}`}>
